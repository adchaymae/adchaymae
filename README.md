# Hi there! 👋 I'm Chaymae Adlane

I'm a **5th-year Computer Engineering** student with a passion for **Artificial Intelligence** and **Web Development**.

---

## 💼 About Me

- 🎓 **Education**:
  - **EMSI** — Computer Engineering with a focus on MIAGE
  - **42 Network** — IT Architecture Expert — Computer Science

- 🌱 **Currently Learning**: Advanced AI techniques and full-stack development

- 💬 **Ask me about**: Web development, AI, or system-level programming

---

## 🛠️ Skills

| Category | Technologies |
|---|---|
| **Languages** | C, C++, Java, C#, JavaScript, Python, SQL |
| **Frameworks & Libraries** | Angular, React, Vue.js, Node.js, Spring Boot, Django |
| **Databases** | MySQL, MongoDB, SQLite |
| **Tools** | Git, Docker, ETL processes |

---

## 📈 Projects

Here are a few highlights from my work:

| # | Project | Tech Stack | Description |
|---|---|---|---|
| 1 | **Voting Application** | React, Node.js (Express), MySQL | Election platform for student club presidents |
| 2 | **Project Management App** | Spring Boot, Vue.js, MySQL | Team project management tool |
| 3 | **School Administration Desktop App** | JavaFX, MySQL, Scene Builder | Streamlines administrative tasks in schools |
| 4 | **My_Books Platform** | Django, Bootstrap, SQLite | User-friendly book management (CRUD) |
| 5 | **Minim-Shell** | C | Custom shell for process and file descriptor management |

---

## 📫 Get in Touch

- 🔗 **LinkedIn**: [Chaymae Adlane](https://www.linkedin.com/in/chaymae-adlane/)
- 📧 **Email**: [chaymaeadlane65@gmail.com](mailto:chaymaeadlane65@gmail.com)

Feel free to explore my repositories and reach out for collaboration or networking!

---

<!---
adchaymae/adchaymae is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
